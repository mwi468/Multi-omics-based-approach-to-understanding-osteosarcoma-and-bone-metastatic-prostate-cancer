...
## also remove baseLine samples 
#x_NEPCremoved_BL_filtered <- x_NEPCremoved[!grepl("-BL", x_NEPCremoved$sampleID), ]
#x_NEPCremoved_BL <- x_NEPCremoved[grepl("-BL", x_NEPCremoved$sampleID), ]
x_NEPCremoved_BL <- x_NEPCremoved

## AR status
filtered_df2 <- survival %>% filter(Subtypes == "AR+/NE+" | Subtypes == "AR+/NE-")

x_ARpos <- x_NEPCremoved_BL[x_NEPCremoved_BL$PatientID %in% filtered_df2$PATIENT_ID,]
x_ARpos$ARstatus <- 'AR+'

x_ARneg <- x_NEPCremoved_BL[!x_NEPCremoved_BL$PatientID %in% filtered_df2$PATIENT_ID,]
x_ARneg$ARstatus <- 'AR-'
...
splitIndex <- createDataPartition(data$ARstatus, p = 0.8, list = FALSE)
...
confMat <- confusionMatrix(predictedClasses, testingData$ARstatus)

## feature selection using random forest
names(data)[names(data) == "NKX3-1"] <- "NKX3_1"
set.seed(123)
rfModel <- randomForest(ARstatus ~ ., data=data, importance=TRUE)
